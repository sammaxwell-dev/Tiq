...
import { Settings, ArrowRightLeft, Copy, Sparkles, ChevronLeft } from 'lucide-react';
        ...
        <div className="flex items-center gap-2">
          {activeTab === 'settings' ? (
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 -ml-2 rounded-full hover:bg-black/5"
          ...
          <span className="font-semibold text-lg tracking-tight">
            {activeTab === 'settings' ? 'Settings' : 'Tippr'}
          </span>
        </div>
        {activeTab === 'translate' && (
          <Button variant="ghost" size="icon" onClick={() => setActiveTab('settings')} className="h-8 w-8 rounded-full hover:bg-black/5">
            <Settings size={18} />
          </Button>
        )}
      </header>
